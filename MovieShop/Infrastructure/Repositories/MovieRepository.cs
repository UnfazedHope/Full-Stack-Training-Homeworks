...
namespace Infrastructure.Repositories
{
    public class MovieRepository : BaseRepository<Movie>, IMovieRepository
    {
        ...
        public async Task<List<Movie>> GetHighestGrossingMovies()
        {
            ...
            var movies = await _dbContext.Movies.OrderByDescending(m => m.Revenue).Take(20).ToListAsync();
            return movies;
        }
        ...
        public override async Task<Movie> GetByIdAsync(int id)
        {
            var movie = await _dbContext.Movies.Include(m => m.MovieCasts).ThenInclude(m => m.Cast)
                .Include(m => m.Genres).Include(m => m.Trailers).FirstOrDefaultAsync(m => m.Id == id);
